...
export class TaskRunner {
  ...
  async run(options: ExecutionOptions): Promise<void> {
    ...
    const tasksToRun = this.configParser.resolveDependencyOrder(taskFile.tasks, options.tasks);

    console.log(`📋 Execution plan: ${tasksToRun.join(' → ')}`);
    console.log('');

    for (const taskName of tasksToRun) {
      const execution = await this.executeTask(taskName, taskFile, options);
      ...
      if (execution.status === 'failed') {
        console.log(`⚠️  Task '${taskName}' failed but continuing due to --continue-on-error`);
        console.log('');
      }
    }
  ...
  private async executeTask(
    ...
    try {
      ...
      if (task.depends_on) {
        for (const dep of task.depends_on) {
          ...
          if (depExecution?.status === 'failed') {
            ...
            console.log(`⏭️  Skipping '${taskName}' due to failed dependency '${dep}'`);
            console.log('');
            return execution;
          }
        }
      ...
      if (options.verbose) {
        ...
        console.log(`   Working dir: ${task.working_directory || process.cwd()}`);
      }
      ...
      if (result.exitCode === 0) {
        ...
        console.log(`❌ Task '${taskName}' failed with exit code ${result.exitCode}`);
  ...
  private interpolateVariables(value: string, env: Record<string, string>): string {
      ...
      executions
        ...
        .forEach(e => {
          const duration = e.endTime && e.startTime ? 
            `${(e.endTime.getTime() - e.startTime.getTime())}ms` : 'unknown';
